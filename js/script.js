/**
 * Premium Armenia Tours - Core Application Controller
 * Handles animations, multi-language transitions, state management,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let activeViewMode = "carousel"; // "carousel" or "grid"
  let activeGridCols = 3; // 2 or 3 columns in grid mode
  ...
  function changeLanguage(lang) {
    ...
    setTimeout(() => {
      ...
      // Handle Armenian typography ligatures for uppercase headings
      ...
      // Re-initialize lucide icons for translated strings
  ...
  // --- SCROLL ANIMATION SYSTEM (RELIABLE) ---
  // Animates children of .scroll-animate sections with stagger.
  // Elements already in viewport on page load get animated immediately.
  ...
  // --- TOUR RENDER & CONTINUOUS MARQUEE ---
  function renderTourCards() {
    if (!toursMarquee) return;

    // Filter Tours
    const filteredTours = toursData.filter(tour => {
      // Support tours that have multiple regions separated by comma e.g. "Kotayk, Gegharkunik"
      ...
      card.innerHTML = `
        <div class="relative w-full ${imgHeightClass} rounded-xl overflow-hidden mb-2 sm:mb-3">
          ...
          <div class="card-region-badge absolute bottom-1.5 left-1.5 sm:bottom-2.5 sm:left-2.5 bg-emerald-light/20 backdrop-blur-md px-1.5 py-0.5 sm:px-3 sm:py-1 rounded-full border border-neon-accent/30 text-[8px] sm:text-xs text-neon-accent font-mono">
            ${translations[currentLang]["filter_region_" + tour.region.toLowerCase()] || tour.region}
          </div>
        </div>
        <div class="flex-grow flex flex-col justify-between">
          ...
          <div class="flex items-center justify-between pt-1.5 border-t border-white/5">
            <div>
              <span class="text-[8px] sm:text-[9px] text-emerald-400 block tracking-wider font-mono">PRICE</span>
              <span class="text-[10px] sm:text-sm font-mono font-bold text-white">${tour.price.toLocaleString()} ${translations[currentLang].tour_amd}</span>
            </div>
    ...
    if (activeViewMode === "carousel") {
      // Carousel - always show immediately, no scroll-animate needed (it's in viewport)
      ...
      // Fade in smoothly after DOM is ready
  ...
  // --- ARMENIAN CHARACTER LIGATURE OVERRIDE ---
  ...
  function closeBurgerDrawer() {
    ...
    // Only clear overflow if details panel is not also open
    const detailsPanel = document.getElementById("details-panel");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (targetElement) {
      ...
      // Use native smooth scroll (polyfilled by JS for control)
      const startY = window.scrollY;
      const distance = scrollTarget - startY;
      const duration = 700; // ms — comfortable, not too fast/slow
      let startTime = null;
  ...
  // --- TOUR DETAILS WORKFLOW & TIMELINE ---
  function openTourDetails(tour) {
    activeSelectedTour = tour;
    
    // Reset Personal Guide Option Toggle
    ...
    // Show the floating close button wrapper
  ...
  function closeTourDetails() {
    ...
    // Only clear overflow if burger drawer is closed
    const burgerDrawer = document.getElementById("burger-drawer");
    ...
    // Hide the floating close button wrapper
    const floatingCloseWrapper = document.getElementById("close-details-floating-wrapper");
    if (floatingCloseWrapper) {
  ...
  function renderTourDetails(tour) {
    ...
    document.getElementById("detail-price").innerHTML = `${tour.price.toLocaleString()} ${translations[currentLang].tour_amd}`;
    ...
    // Load Price Calculator initial parameters
    ...
    stops.forEach((stop, index) => {
      ...
      stopNode.addEventListener("click", () => {
        isAutoplayPaused = true; // Pause autoplay on manual interactions
    ...
    // Populate initial stop active info
    showStop(0, stops);

    // Setup Autoplay Timer (Every 5 seconds)
    clearInterval(detailAutoplayTimer);
  ...
  function showStop(index, stops) {
    ...
    // Transition large image & background smoothly
    ...
    // Dynamically update the main title and description of the details screen with active stop info
    ...
    // Toggle active state visual on sidebar list stops
    ...
    // Handle Landmark card overlay inside image corner
    ...
    // Update dynamic top timeline bar
  ...
  // --- EXPANDABLE BOOKING SOCIALS PANEL ---
  ...
  // Prefill Social Chats (WhatsApp prefilled template)
  ...
  function updateCalculator() {
    ...
    const basePrice = activeSelectedTour.price;
    let finalPricePerPerson = basePrice;
    let labelKey = "calc_type_private";

    // Dynamic calculations
    if (peopleCount >= 4 && peopleCount <= 7) {
      finalPricePerPerson = basePrice * 0.85; // 15% discount for group
      labelKey = "calc_type_group";
    } else if (peopleCount >= 8) {
      finalPricePerPerson = basePrice * 0.70; // 30% discount for large group
      labelKey = "calc_type_discount";
    }

    // Personal Guide Surcharge (+20000 AMD flat)
    const guideToggle = document.getElementById("calc-guide-toggle");
    ...
    // Update Counter Animation (Ticking numbers up/down)
  ...
  // Smooth Numeric Ticking Animation
  ...
  // --- BENTO GRID TEAM RENDER ---
  const teamContainer = document.getElementById("team-bento-container");
  const teamData = [
    {
      id: "arsen",
      name: { hy: "Լուսինե", ru: "Лусине", en: "Lusine" },
      role: { hy: "Հիմնադիր եվ Ավագ Գիդ", ru: "Основатель и Старший Гид", en: "Founder & Senior Guide" },
      funnyDesc: {
        hy: "Գիտի Հայաստանի բոլոր լեռնային ճանապարհները անգիր: Կարող է լավաշ թխել նույնիսկ փակ աչքերով և պատմել հազարամյա վանքերի մասին ժամեր շարունակ:",
        ru: "Знает все горные тропы Армении наизусть. Может испечь лаваш с закрытыми глазами и увлеченно рассказывать о древних храмах часами.",
        en: "Knows every mountain pass by heart. Can bake traditional lavash with his eyes closed and talk about medieval history for hours."
      },
      image: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=400" // Premium corporate/outdoors avatar
    },
    {
      id: "ani",
      name: { hy: "Անի", ru: "Ани", en: "Ani" },
      role: { hy: "Լեռնային էքստրեմալ ուղեկցորդ", ru: "Горный экстрим-проводник", en: "Mountain Extreme Leader" },
      funnyDesc: {
        hy: "Անցել է Կովկասյան լեռները ոտքով երեք անգամ: Նրանից չի թաքնվի ոչ մի գեղեցիկ համայնապատկեր, և նա միշտ պատրաստ է անակնկալ սուրճ պատրաստել անտառի մեջ:",
        ru: "Прошла Кавказские хребты пешком трижды. От нее не скроется ни одна панорама, а в рюкзаке всегда найдется пара джезв для кофе в лесу.",
        en: "Hiked the entire Caucasus ridge three times. No scenic viewpoint can escape her, and she always hides a traditional coffee pot in her bag."
      },
      image: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=400"
    },
    {
      id: "gor",
      name: { hy: "Գոռ", ru: "Гор", en: "Gor" },
      role: { hy: "Գաստրո և Գինե Էքսպերտ", ru: "Эксперт по Гастрономии и Вину", en: "Gastro & Wine Sommelier" },
      funnyDesc: {
        hy: "Կարող է տարբերել Արենի գինիների բերքահավաքի տարեթիվը միայն հոտով: Պատրաստում է երկրագնդի ամենահամեղ խորովածը լեռան գագաթին:",
        ru: "Определяет сорт и год урожая вина Арени по одному аромату. Готовит самый сочный шашлык прямо на вершинах ветреных перевалов.",
        en: "Distinguishes the exact harvest year of Areni grapes blindfolded. Master of cooking the juiciest kebab on windy mountain tops."
      },
  ...
  function renderTeamBento() {
    ...
    teamData.forEach((member, index) => {
      ...
      // Tap highlight animation effect - crisp scale, shadow, and ring border
    ...
    // Re-check if team section is now visible and trigger animation
  ...
  function animateCounters() {
    counters.forEach(c => {
      ...
      function update(timestamp) {
        ...
        // Quad-ease-out formula for counting feel
  ...
  const reviewsData = [
    {
      ...
      review: {
        hy: "Անմոռանալի փորձ! Տաթևի թռիչքը և Նորավանքի գեղեցկությունը կտրում էին շունչս: Գիդերը պրոֆեսիոնալ էին, իսկ ամենագնացը՝ շատ հարմարավետ:",
        ru: "Невероятный опыт! Полет в Татев и багряные скалы Нораванка захватывали дух. Гиды были супер-профессиональны, а джипы очень комфортными.",
        en: "An absolutely breath-taking experience! Flying to Tatev and standing in Noravank's red canyon was surreal. Deeply professional service."
      },
      stars: 5,
    ...
    },
    {
      ...
      country: "Russia",
      review: {
        hy: "Գերազանց տուր դեպի Դիլիջան և Պարզ լիճ: Գաստրո համտեսները հիասքանչ էին: Բարձրակարգ սպասարկում և ջերմ մթնոլորտ:",
        ru: "Шикарный тур в Дилижан и на озеро Парз. Гастрономическая часть превзошла все ожидания! Действительно премиальный уровень сервиса.",
        en: "Excellent forest trip to Dilijan & Parz Lake. The food and wine tasting portion blew our minds. High-end comfort and safety standards."
      },
      stars: 5,
    ...
    },
    {
      ...
      country: "France",
      review: {
        hy: "Հայաստանը զարմացրեց ինձ իր պատմությամբ, իսկ այս թիմը ստեղծեց կատարյալ հարմարավետություն: Գառնի-Գեղարդ տուրը հիասքանչ էր:",
        ru: "Армения поразила своей историей, а команда устроила люксовое путешествие. Храм Гарни и Симфония камней останутся в памяти навсегда.",
        en: "Armenia amazed me with its history, and this agency curated a luxury adventure. Garni Temple and Symphony of Stones is a must-see."
      },
      stars: 5,
  ...
  // --- INITIAL BOOTSTRAP ---
  // --- PREMIUM LIGHT/DARK THEME CONTROLLER ---
  const themeToggleBtn = document.getElementById("theme-toggle");
  const themeIcon = document.getElementById("theme-icon");

  // Load user's saved preference, defaulting to dark theme
  ...
  function applyTheme(theme) {
    ...
    // Re-create icons to reflect correct lucide state
    if (typeof lucide !== "undefined") {
      lucide.createIcons();
    }
  }
  ...
  // --- INTERACTIVE DRAG/SWIPE CAROUSEL SCROLLER ---
  ...
  function initCarouselInteraction() {
    ...
    // Smooth physics and auto-scroll recovery animation frame loop
      ...
      if (activeViewMode === "carousel" && marquee.children.length > 0) {
        ...
        if (!isDragging) {
          if (Math.abs(velX) > 0.1) {
            ...
            // Auto-scroll acceleration curve
          ...
          // Infinite loop wrap point calculations
  ...
  // Dynamic parallax elements floating values
  ...
  // Call dynamic i18n triggers initially
